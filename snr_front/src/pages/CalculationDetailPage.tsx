...
export const CalculationDetailPage: React.FC = () => {
  ...
  const handleUpdateItem = async (idx: number) => {
    ...
    setLoading(true);
    try {
      await apiClient.put(`/api/model_calculation/${modelId}`, payload);
    } catch (err: any) {
      console.error(err);
  ...
  const handleRemoveItem = async (modelId: number) => {
    setLoading(true);
    try {
      await apiClient.delete(`/api/model_calculation/${modelId}`);
  ...
  const handleDeleteCalculation = async () => {
    ...
    setLoading(true);
    await apiClient.delete(`/api/nuclear_calculations/${id}`);
    dispatch(fetchDraftSummary());
  ...
  const handleFormCalculation = async () => {
    setLoading(true);
    await apiClient.put(`/api/nuclear_calculations/${id}/form`);
    dispatch(fetchDraftSummary());
    navigate(ROUTES.CALCULATIONS);
  };
  ...
  const totalNumbersDisplay = useMemo(() => {
    ...
    localItems.forEach(item => {
      const { res_power, res_fuel } = calculateModel(Number(item.power), Number(item.fuel), Number(item.amount));
  ...
  const status = calcData.calc.status;
                  ...
                  {localItems.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="align-middle text-center">
                        ...
                        <td>
                          <input 
                            type="number" className="input" style={{ width: '100px' }}
                            value={item.amount} onChange={e => handleFieldChange(idx, 'amount', e.target.value)} step="any" readOnly={!isDraft || loading} min="1"
                        ...
                        {isDraft && (
                          <td>
                            <div className="d-flex gap-2 justify-content-center">
                              <button 
                                className="butn-small" title="Сохранить изменения" disabled={loading}
                                onClick={() => handleUpdateItem(idx)}
                              >
                                Обновить
                              </button>
                              <button 
                                className="butn-danger-small" title="Удалить диапазон" disabled={loading}
